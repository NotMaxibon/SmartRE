...
public class MTCNN {
    ...
    private Bitmap bitmapResize(Bitmap bm, float scale) {
        ...
        // CREATE A MATRIX FOR THE MANIPULATION。matrix Specify picture affine transformation parameters
    ...
    private  int PNetForward(Bitmap bitmap,float [][]PNetOutProb,float[][][]PNetOutBias){
        ...
        //
        //[write one] first flip, then turn into a 2/3 dimension array
        ...
        /*
        *[Writing 2] This is faster, 3ms faster. Not meaningful, use the above method to be more intuitive
    ...
    //nms，Unqualified deleted is set to true
    private void nms(Vector<Box> boxes,float threshold,String method){
        ...
        for(int i=0;i<boxes.size();i++) {
            Box box = boxes.get(i);
            if (!box.deleted) {
                //score<0Indicates that the current rectangle is deleted.
                for (int j = i + 1; j < boxes.size(); j++) {
                    ...
                    if (!box2.deleted) {
                        ...
                        if (iou >= threshold) {
                            //Delete the small box with prob
    ...
    /*
Execute after the NMS is executed.Regression
    ...
    private Vector<Box> PNet(Bitmap bitmap,int minSize){
        ...
        while (currentFaceSize<=whMin){
            ...
            //Face Size Equivalent increment
    ...
    //Intercept the rectangle specified in the box (over the boundary to be processed), and resize to size * size, the return data is stored in the data.
     ...
     * minFaceSize: the smallest face pixel value. (The larger the value, the faster the detection)
     * Back:
     * Face frame
     */
